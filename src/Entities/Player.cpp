...
void ind::Player::draw()
{
    ...
    const irr::core::vector3df actualPosition = this->object->getPosition();
...
{
    const irr::core::vector3df horizontalNextPos = irr::core::vector3df(actualPosition.X, actualPosition.Y, actualPosition.Z - this->force.X);
    const irr::core::vector3df verticalNextPos = irr::core::vector3df(actualPosition.X - this->force.Y, actualPosition.Y, actualPosition.Z);
    const irr::core::vector3df nextPos = irr::core::vector3df(actualPosition.X - this->force.Y, actualPosition.Y, actualPosition.Z - this->force.X);
...
const bool ind::Player::isWalkable(const irr::core::vector3df &position, const irr::core::vector3df &direction)
{
    const irr::core::vector3df posBorder = position - direction * (TILE_SIZE / 2);
    const irr::core::vector3df cornerOffset = (direction.X != 0 ? irr::core::vector3df(0, 0, TILE_SIZE / 2.3) : irr::core::vector3df(TILE_SIZE / 2.3, 0, 0));
    const Position firstCorner = to2d(posBorder + cornerOffset);
    const Position secondCorner = to2d(posBorder - cornerOffset);
    const Position mapSize = this->map.getSize();
    ...
    const Tiles firstCornerTile = this->map.getInfoAtCoord(firstCorner);
    const Tiles secondCornerTile = this->map.getInfoAtCoord(secondCorner);
